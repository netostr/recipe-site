...
const User = z.object({
    name: z.string()
        .trim()
        .min(1, "Пожалуйста введите логин"),
    email: z.email({
        error: (iss) => !iss.input ? "Пожалуйста введите адрес электронной почты" : "Указан неверный адрес электронной почты"
    }).toLowerCase().trim(),
    password: z.string()
        .min(16, {
            error: (iss) => {
...
export const regisration = validatedAction(User, async (data, formData) => {
    const { name, email, password, recaptcha_token } = data;

    const { success: isRecaptchaValid, score } = await verifyRecaptchaV3(recaptcha_token)
    ...
    if (!isRecaptchaValid) {
        ...
        return {
            data,
            errors: {
                recaptcha_token: ["Проверка безопасности не пройдена. Попробуйте ещё раз."],
            }
        };
    }
